...
admin_blueprint = Blueprint(
    "admin_blueprint", __name__, template_folder="templates", static_folder="static"
)
...
def add_artefact():
    if request.method == "POST":
        ...
        return {"success": True, "next": ARTEFACTS[idx + 1]}
...
def add_artefact_material():
    if request.method == "POST":
        ...
        if materials != "":
            ...
            materials_dict = {
                int(materials[i]): int(materials[i + 1])
            ...
            try:
                for material_id, material_amt in materials_dict.items():
                    ...
                    artefact = (
                        session.query(Artefact).filter_by(id=r["artefactId"]).first()
                    )
                    if artefact is None:
                    ...
                    engine.execute(
                        sql_statement
                    )  # SQLite is too wimpy to have two concurrent processes
            ...
            return {"success": True, "msg": f"Succesfully updated {artefact_name}"}, 200

        return {"success": False, "error": "No materials specified"}, 400
    ...
    if request.method == "POST":
        ...
        try:
            for artefact_reward in artefact_rewards:
                ...
                collection = (
                    session.query(Collection).filter_by(id=r["collectionId"]).first()
                )
                ...
                artefact = (
                    ...
                    .filter_by(id=artefact_reward["artefactId"])
                    .first()
                )
                if artefact is None:
                    raise Exception(
                        f'No Artefact with id={artefact_reward["artefactId"]} found'
                    )

                reward = (
                    session.query(Reward)
                    .filter_by(id=artefact_reward["rewardId"])
                    .first()
                )
                if reward is None:
                    raise Exception(
                        f'No Reward with id={artefact_reward["rewardId"]} found'
                    )
            ...
            for collection_reward in collection_rewards:
                ...
                reward = (
                    session.query(Reward)
                    .filter_by(id=collection_reward["rewardId"])
                    .first()
                )
                if reward is None:
                    raise Exception(
                        f'No Reward with id={collection_reward["rewardId"]} found'
                    )
                ...
                sql_statement = r_c.insert().values(
        ...
        return {"success": True, "msg": f"Succesfully updated {collection_name}"}, 200
